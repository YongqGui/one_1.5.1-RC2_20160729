...
public class DTNSim {
	...
	public static void main(String[] args) {
		...
		SatelliteOrbit saot;
		saot=new SatelliteOrbit();                     // 得到卫星运行轨迹路径
		coordinate = saot.getSatelliteCoordinate(tf);
		System.out.print("hello\n");
		double a=12;
		//System.out.print(coordinate[1][3]);
        System.out.print("hello1 : " + a + "\n");
		for(int i=0;i<3;++i) {
			System.out.println("第 " + (i+1) +"个参数 ："+coordinate[0][i]+ "\n");
		}
		...
		initSettings(confFiles, firstConfIndex);//confFiles 在这里是default_setting.txt
		
		if (batchMode) {
			long startTime = System.currentTimeMillis();     //以毫秒级返回当前时间
			for (int i=nrofRuns[0]; i<nrofRuns[1]; i++) {
				print("Run " + (i+1) + "/" + nrofRuns[1]);
				Settings.setRunIndex(i);
				resetForNextRun();
				new DTNSimTextUI().start(); //TestUI
			}
