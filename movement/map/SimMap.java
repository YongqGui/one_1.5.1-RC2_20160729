...
public class SimMap implements Serializable {
	private Coord minBound; //最小界
	private Coord maxBound;//最大界
	/** list representation of the map for efficient list-returning */
	private ArrayList<MapNode> nodes;  //地图节点
	/** hash map presentation of the map for efficient finding node by coord */
	private Map<Coord, MapNode> nodesMap;  // 有位置坐标和地图节点？？  世界图吗？
	...
	public SimMap(Map<Coord, MapNode> nodes) {//仿真图构造函数
		this.offset = new Coord(0,0);  //位置为（0，0）
		this.nodes = new ArrayList<MapNode>(nodes.values());  //地图节点坐标
		this.nodesMap = nodes;
