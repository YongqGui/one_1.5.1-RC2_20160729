...
public abstract class ActiveRouter extends MessageRouter {
	...
	public int receiveMessage(Message m, DTNHost from) {
		int recvCheck = checkReceiving(m, from); //妫�煡浜�/router isn't transferring, doesn't have the message and has room for i
		if (recvCheck != RCV_OK) {
			return recvCheck;
		}
	...
	protected int startTransfer(Message m, Connection con) {
		...
		if (!policy.acceptSending(getHost(), 			// getHost()返回当前节点
				con.getOtherNode(getHost()), con, m)) {	//默认情况，所有消息都接受。 支持三种策略：simple policy, Hop Count, ModuleCommunicationBus(MCB)
			return MessageRouter.DENIED_POLICY;  		//这种情况是什么意思？？？？？？？不明白（节点是低值，不好的？？？？）
		}
	...
	protected void dropExpiredMessages() {
		...
		for (int i=0; i<messages.length; i++) {
			...
			if (ttl <= 0) {
				...
				System.out.println("删除消息："+" "+messages[i].getId()+"  "+messages[i].getTo()+"  "+messages[i].getFrom());
			}
		}
	}	
	/**
	...
	public boolean isTransferring() {
		if (this.sendingConnections.size() > 0) {//鎯呭舰1锛氭湰鑺傜偣姝ｅ湪浼犺緭
			return true; // sending something
		}
		
		List<Connection> connections = getConnections();
		
		if (connections.size() == 0) {//鎯呭瀷2锛氭病鏈夐偦灞呰妭鐐�
			return false; // not connected
		}
		//鎯呭瀷3锛氭湁閭诲眳鑺傜偣锛屼絾鏈夐摼璺鍦ㄤ紶杈�
		for (int i=0, n=connections.size(); i<n; i++) {
			Connection con = connections.get(i);
	...
	public void update() {		
		...
		for (int i=0; i<this.sendingConnections.size(); ) {			//对于每条链接都遍历一遍
			boolean removeCurrent = false;
			Connection con = sendingConnections.get(i);  			//得到其中一条链接
			
			/* finalize ready transfers */
			if (con.isMessageTransferred()) {    		// 消息是否传输完毕，即是否过了(1.0*m.getSize())/this.speed没 // 选中的这条连接中，是否有消息正在传输// 该链路可以传输消息
				if (con.getMessage() != null) {  		// 这条链接中有消息
					transferDone(con);           		// 完成传递消息的函数
